...
var _ = Describe("Commands", func() {
	...
	Describe("server", func() {
		...
		It("should Wait", func() {
			...
			// assume testing on single redis-set instance
			start := time.Now()
	...
	Describe("keys", func() {
		...
		It("should Sort and Get", func() {
			...
			{
				els, err := client.SortInterfaces("list", &Sort{
					Get: []string{"object_*"},
				}).Result()
				Expect(err).NotTo(HaveOccurred())
		...
		It("should Sort and Store", func() {
			...
			n, err := client.SortStore("list", "list2", &Sort{
				Offset: 0,
				Count:  2,
				Order:  "ASC",
			}).Result()
	...
	Describe("strings", func() {
		...
		It("should BitCount", func() {
			...
			bitCount = client.BitCount("key", &BitCount{
				Start: 0,
				End:   0,
			})
			Expect(bitCount.Err()).NotTo(HaveOccurred())
	...
	Describe("sorted sets", func() {
		...
		It("should ZAddNX", func() {
			added, err := client.ZAddNX("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddXX", func() {
			added, err := client.ZAddXX("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddCh", func() {
			changed, err := client.ZAddCh("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddNXCh", func() {
			changed, err := client.ZAddNXCh("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZAddXXCh", func() {
			changed, err := client.ZAddXXCh("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncr", func() {
			...
			score, err = client.ZIncr("zset", Z{Score: 1, Member: "one"}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncrNX", func() {
			score, err := client.ZIncrNX("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZIncrXX", func() {
			score, err := client.ZIncrXX("zset", Z{
				Score:  1,
				Member: "one",
			}).Result()
			Expect(err).To(Equal(Nil))
		...
		It("should ZInterStore", func() {
			...
			err = client.ZAdd("zset2", Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset2", Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset3", Z{Score: 3, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())

			zInterStore := client.ZInterStore(
				"out", ZStore{Weights: []float64{2, 3}}, "zset1", "zset2")
		...
		It("should ZRange", func() {
			err := client.ZAdd("zset", Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset", Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset", Z{Score: 3, Member: "three"}).Err()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRangeByScore", func() {
			...
			zRangeByScore := client.ZRangeByScore("zset", ZRangeBy{
				Min: "-inf",
				Max: "+inf",
			})
		...
		It("should ZRangeByLex", func() {
			...
			zRangeByLex := client.ZRangeByLex("zset", ZRangeBy{
				Min: "-",
				Max: "+",
			})
		...
		It("should ZRangeByScoreWithScoresMap", func() {
			...
			vals, err := client.ZRangeByScoreWithScores("zset", ZRangeBy{
				Min: "-inf",
				Max: "+inf",
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRevRangeByScore", func() {
			...
			vals, err := client.ZRevRangeByScore(
				"zset", ZRangeBy{Max: "+inf", Min: "-inf"}).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(vals).To(Equal([]string{"three", "two", "one"}))

			vals, err = client.ZRevRangeByScore(
				"zset", ZRangeBy{Max: "2", Min: "(1"}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZRevRangeByLex", func() {
			err := client.ZAdd("zset", Z{Score: 0, Member: "a"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset", Z{Score: 0, Member: "b"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset", Z{Score: 0, Member: "c"}).Err()
			Expect(err).NotTo(HaveOccurred())

			vals, err := client.ZRevRangeByLex(
				"zset", ZRangeBy{Max: "+", Min: "-"}).Result()
			Expect(err).NotTo(HaveOccurred())
			Expect(vals).To(Equal([]string{"c", "b", "a"}))

			vals, err = client.ZRevRangeByLex(
				"zset", ZRangeBy{Max: "[b", Min: "(a"}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should ZScore", func() {
			zAdd := client.ZAdd("zset", Z{Score: 1.001, Member: "one"})
			Expect(zAdd.Err()).NotTo(HaveOccurred())
		...
		It("should ZUnionStore", func() {
			err := client.ZAdd("zset1", Z{Score: 1, Member: "one"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset1", Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			...
			err = client.ZAdd("zset2", Z{Score: 2, Member: "two"}).Err()
			Expect(err).NotTo(HaveOccurred())
			err = client.ZAdd("zset2", Z{Score: 3, Member: "three"}).Err()
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("streams", func() {
		...
		It("should XRead", func() {
			res, err := client.XRead(&XReadArgs{
		...
		Describe("group", func() {
			BeforeEach(func() {
				...
				res, err := client.XReadGroup(&XReadGroupArgs{
					Group:    "group",
					Consumer: "consumer",
			...
			It("should XPending", func() {
				...
				infoExt, err := client.XPendingExt(&XPendingExtArgs{
					Stream:   "stream",
					Group:    "group",
					Start:    "-",
					End:      "+",
			...
			It("should XClaim", func() {
				msgs, err := client.XClaim(&XClaimArgs{
					Stream:   "stream",
					Group:    "group",
					Consumer: "consumer",
				...
				ids, err := client.XClaimJustID(&XClaimArgs{
					Stream:   "stream",
					Group:    "group",
					Consumer: "consumer",
					Messages: []string{"1-0", "2-0", "3-0"},
	...
	Describe("Geo add and radius search", func() {
		...
		It("should search geo radius", func() {
			res, err := client.GeoRadius("Sicily", 15, 37, &GeoRadiusQuery{
				Radius: 200,
			}).Result()
			Expect(err).NotTo(HaveOccurred())
		...
		It("should search geo radius by member with options", func() {
			res, err := client.GeoRadiusByMember("Sicily", "Catania", &GeoRadiusQuery{
				Radius:      200,
				Unit:        "km",
				WithGeoHash: true,
				WithCoord:   true,
		...
		It("should search geo radius with no results", func() {
			res, err := client.GeoRadius("Sicily", 99, 37, &GeoRadiusQuery{
				Radius:      200,
				Unit:        "km",
				WithGeoHash: true,
				WithCoord:   true,
