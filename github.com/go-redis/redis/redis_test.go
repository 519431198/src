...
var _ = Describe("Client", func() {
	...
	It("should close", func() {
		...
		err := client.Ping().Err()
		Expect(err).To(MatchError("redis-set: client is closed"))
	})
	...
	It("should select DB", func() {
		...
		Expect(db2.Get("db").Err()).To(Equal(Nil))
		...
		Expect(client.Get("db").Err()).To(Equal(Nil))
	...
	It("should call WrapProcess", func() {
		...
		client.WrapProcess(func(old func(Cmder) error) func(Cmder) error {
	...
	It("should call WrapProcess after WithContext", func() {
		...
		client.WrapProcess(func(old func(cmd Cmder) error) func(cmd Cmder) error {
			return func(cmd Cmder) error {
				fn1Called = true
				return old(cmd)
			}
		...
		client2.WrapProcess(func(old func(cmd Cmder) error) func(cmd Cmder) error {
			return func(cmd Cmder) error {
				fn2Called = true
				return old(cmd)
			}
...
var _ = Describe("Client OnConnect", func() {
	...
	BeforeEach(func() {
		opt := redisOptions()
		opt.DB = 0
		opt.OnConnect = func(cn *Conn) error {
