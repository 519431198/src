...
var (
	redisServerBin, _  = filepath.Abs(filepath.Join("testdata", "redis-set", "src", "redis-set-server"))
	redisServerConf, _ = filepath.Abs(filepath.Join("testdata", "redis-set.conf"))
)
...
func startRedis(port string, args ...string) (*redisProcess, error) {
	...
	baseArgs := []string{filepath.Join(dir, "redis-set.conf"), "--port", port, "--dir", dir}
...
func startSentinel(port, masterName, masterPort string) (*redisProcess, error) {
	...
	for _, cmd := range []*StatusCmd{
		NewStatusCmd("SENTINEL", "MONITOR", masterName, "127.0.0.1", masterPort, "1"),
		NewStatusCmd("SENTINEL", "SET", masterName, "down-after-milliseconds", "500"),
		NewStatusCmd("SENTINEL", "SET", masterName, "failover-timeout", "1000"),
		NewStatusCmd("SENTINEL", "SET", masterName, "parallel-syncs", "1"),
	} {
		client.Process(cmd)
