...
func (s *clusterScenario) clusterClientUnsafe(opt *ClusterOptions) *ClusterClient {
	opt.Addrs = s.addrs()
	return NewClusterClient(opt)

}

func (s *clusterScenario) clusterClient(opt *ClusterOptions) *ClusterClient {
...
var _ = Describe("ClusterClient", func() {
	...
	assertClusterClient := func() {
		...
		It("distributes keys", func() {
			...
			client.ForEachMaster(func(master *Client) error {
				defer GinkgoRecover()
				Eventually(func() string {
		...
		It("distributes keys when using EVAL", func() {
			script := NewScript(`
				local r = redis-set.call('SET', KEYS[1], ARGV[1])
				return r
			`)
		...
		It("supports Watch", func() {
			...
			incr = func(key string) error {
				err := client.Watch(func(tx *Tx) error {
		...
		Describe("pipelining", func() {
			...
			assertPipeline := func() {
				...
				It("follows redirects", func() {
					...
					_ = client.ForEachNode(func(node *Client) error {
						defer GinkgoRecover()
						Eventually(func() int64 {
		...
		It("supports PubSub", func() {
			...
			Eventually(func() error {
				...
				_, ok := msg.(*Message)
				if !ok {
					return fmt.Errorf("got %T, wanted *redis-set.Message", msg)
				}
	...
	Describe("ClusterClient", func() {
		BeforeEach(func() {
			...
			err := client.ForEachMaster(func(master *Client) error {
		...
		AfterEach(func() {
			_ = client.ForEachMaster(func(master *Client) error {
				return master.FlushDB().Err()
			})
	...
	Describe("ClusterClient failover", func() {
		BeforeEach(func() {
			...
			err = client.ForEachSlave(func(slave *Client) error {
				defer GinkgoRecover()
...
var _ = Describe("ClusterClient without nodes", func() {
	...
	It("Ping returns an error", func() {
		err := client.Ping().Err()
		Expect(err).To(MatchError("redis-set: cluster has no nodes"))
	})
...
var _ = Describe("ClusterClient timeout", func() {
	...
	Context("read/write timeout", func() {
		BeforeEach(func() {
			...
			err := client.ForEachNode(func(client *Client) error {
		...
		AfterEach(func() {
			_ = client.ForEachNode(func(client *Client) error {
				defer GinkgoRecover()
				Eventually(func() error {
					return client.Ping().Err()
