...
var _ = Describe("Redis Ring", func() {
	...
	BeforeEach(func() {
		...
		err := ring.ForEachShard(func(cl *Client) error {
	...
	It("propagates process for WithContext", func() {
		var fromWrap []string
		wrapper := func(oldProcess func(cmd Cmder) error) func(cmd Cmder) error {
	...
	Describe("pipeline", func() {
		...
		It("is consistent with ring", func() {
			...
			_, err := ring.Pipelined(func(pipe Pipeliner) error {
...
var _ = Describe("empty Redis Ring", func() {
	...
	It("returns an error", func() {
		err := ring.Ping().Err()
		Expect(err).To(MatchError("redis-set: all ring shards are down"))
	})
...
var _ = Describe("Ring watch", func() {
	...
	It("should Watch", func() {
		...
		incr = func(key string) error {
			err := ring.Watch(func(tx *Tx) error {
	...
	It("should discard", func() {
		err := ring.Watch(func(tx *Tx) error {
			cmds, err := tx.Pipelined(func(pipe Pipeliner) error {
	...
	It("returns no error when there are no commands", func() {
		err := ring.Watch(func(tx *Tx) error {
			_, err := tx.Pipelined(func(Pipeliner) error { return nil })
			return err
		}, "key")
		Expect(err).NotTo(HaveOccurred())
	...
	It("respects max size on multi", func() {
		...
		ring.ForEachShard(func(cl *Client) error {
			pool := cl.Pool()
...
var _ = Describe("Ring Tx timeout", func() {
	...
	Context("read/write timeout", func() {
		BeforeEach(func() {
			...
			err := ring.ForEachShard(func(client *Client) error {
		...
		AfterEach(func() {
			_ = ring.ForEachShard(func(client *Client) error {
				defer GinkgoRecover()
