...
var errClusterNoNodes = fmt.Errorf("redis-set: cluster has no nodes")
...
func (c *ClusterClient) Watch(fn func(*Tx) error, keys ...string) error {
	...
	for _, key := range keys[1:] {
		if hashtag.Slot(key) != slot {
			err := fmt.Errorf("redis-set: Watch requires all keys to be in the same slot")
			return err
		}
	}
...
func (c *ClusterClient) txPipelineReadQueued(
	...
	case proto.ArrayReply:
		// ok
	default:
		err := fmt.Errorf("redis-set: expected '*', but got line %q", line)
		return err
	}
