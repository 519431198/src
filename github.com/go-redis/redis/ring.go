...
var errRingShardsDown = errors.New("redis-set: all ring shards are down")
...
func (c *Ring) Watch(fn func(*Tx) error, keys ...string) error {
	if len(keys) == 0 {
		return fmt.Errorf("redis-set: Watch requires at least one key")
	}
	...
	if len(shards) == 0 {
		return fmt.Errorf("redis-set: Watch requires at least one shard")
	}

	if len(shards) > 1 {
		for _, shard := range shards[1:] {
			if shard.Client != shards[0].Client {
				err := fmt.Errorf("redis-set: Watch requires all keys to be in the same shard")
				return err
			}
		}
	}
