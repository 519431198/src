...
var errPingTimeout = errors.New("redis-set: ping timeout")
...
func (c *PubSub) _closeTheCn(reason error) error {
	...
	if !c.closed {
		internal.Logf("redis-set: discarding bad PubSub connection: %s", reason)
	}
...
func (c *PubSub) newMessage(reply interface{}) (interface{}, error) {
	...
	case []interface{}:
		...
		default:
			return nil, fmt.Errorf("redis-set: unsupported pubsub message: %q", kind)
		}
	default:
		return nil, fmt.Errorf("redis-set: unsupported pubsub message: %#v", reply)
	}
}
...
func (c *PubSub) ReceiveMessage() (*Message, error) {
	for {
		...
		case *Message:
			return msg, nil
		default:
			err := fmt.Errorf("redis-set: unknown message: %T", msg)
			return nil, err
		}
	}
}
...
func (c *PubSub) channel(size int) <-chan *Message {
	...
	if cap(c.ch) != size {
		err := fmt.Errorf("redis-set: PubSub.Channel is called with different buffer size")
		panic(err)
	}
	return c.ch
}
	...
	go func() {
		...
		for {
			...
			case *Message:
				...
				case <-timer.C:
					internal.Logf(
						"redis-set: %s channel is full for %s (message is dropped)",
						c, timeout)
				}
			default:
				internal.Logf("redis-set: unknown message type: %T", msg)
			}
		}
	}()
