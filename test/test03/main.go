...
func main() {
	...
	configPath = "/Users/wangyi/goProject/project_1/src/company/zhejiang/config.yaml"
	...
	case 1:
		...
		resFilePath = fmt.Sprintf("test/test03/%s.csv", time.Now().AddDate(0, 0, -1).Format("2006-01-02"))
		config.yesterday(bill, resFilePath)
	case 2:
		resFilePath := path + config.StartTime + config.EndTime
		resFilePath = fmt.Sprintf("test/test03/%s-->%s.csv", config.StartTime, config.EndTime)
