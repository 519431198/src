...
func main() {
	now := time.Now()
	// 获取前一天时间
	oldTime := now.AddDate(0, 0, -1).Format("15:04:05")
	...
	if now.Format("15:04") == "21:15" {
		oldTime = now.AddDate(0, 0, -1).Format("2006-01-02")
		table = "bill_hainan_" + now.AddDate(0, 0, -1).Format("20060102")
		resDataPath = "/Users/wangyi/goProject/project_1/src/company/zhejiang/hnzj_bill.csv"
		//resDataPath = filepath.Join(fmt.Sprintf("%s/%s.csv", binPath, "hnzj_bill.csv"))
	}
	...
	//file, err := os.Open("D:\\go\\src\\company\\hainan\\config.yaml")
