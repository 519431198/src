...
// GetSql 获取 sql 字符串
func GetSql(Customer Customer, test Test, t string) string {
	var phone string
	...
	form := fmt.Sprintf("bill_ware_house_" + t)
	// 写 sql 语句
	...
	//sqlStr := fmt.Sprintf("SELECT ? as customer_name,sum(charge6),sum(charge60) FROM `bill_ware_house_2023-06-24` WHERE phone_number_x IN (%s) AND charge60 != '0'", phone)
	return sqlStr
}
